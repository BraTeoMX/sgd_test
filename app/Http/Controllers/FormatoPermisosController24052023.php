...
use App\Calendario;
use App\Faltas;
use App\Imports\FormatoPermisosImport;
use DB;
...
class FormatoPermisosController extends Controller
{
        ...
        if ($per_nivel->Nivel == 1) {
            ...
            if (auth()->user()->hasRole("Coordinador/Analista")) {
                ...
                /* if (auth()->user()->hasRole("Vicepresidente")){
                        FormatoPermisos::WHERE('folio_per',$per->folio_per)->UPDATE(['permiso_jefe'=>1,'status'=>'APLICADO','fecha_aprobacion'=>date("Y-m-d"),'excepcion'=>1]);
                    }else{ */
                FormatoPermisos::WHERE('folio_per', $per->folio_per)->UPDATE(['permiso_jefe' => 1, 'status' => 'APLICADO', 'fecha_aprobacion' => date("Y-m-d"), 'excepcion' => $per->excepcion]);
                // }
    ...
    {
        return view('formatopermisos.masivos');
    }

    public function guardarmasivos(Request $request)
    {
        #echo 'hola para guardar los masivos';
        $import = new FormatoPermisosImport;
        $request->validate([
            'file' => 'required|mimes:xlsx'
        ]);
        $file = $request->file('file');
        if (Excel::import($import, $file)) {
            flash::success('Se han guardado los permisos masivos')->important();
        } else {
            flash::error('Ocurrió un problema al intentar cargar permisos masivos')->important();
        }
        #Excel::import($import, $file);
        #echo 'Se importó';
        #return view('formatopermisos.masivos')->with('exito', 'ok');
    }
}
