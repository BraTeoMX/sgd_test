    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            @forelse($datosEmpleado as $vacacion)
                ...
                {!! BootForm::hidden('id_dep', $vacacion->Departamento); !!}
                ...
                <div class="row">
                    ...
                    <div class="col-lg-3 col-md-3">
                        {!! BootForm::text('turno', 'TURNO ', $vacacion->Id_Turno, ['width'=>'col-md-3','readonly']); !!}
                    </div>
                </div>
                <div class="row">
                ...
                @if ($vacacion->Modulo != '' || auth()->user()->hasRole('Coordinador/Analista'))
                <div class="row">
                    <div class="col-lg-6 col-md-6">
                                    ...
                                    @if($i->id_puesto_solicitante != $i->id_jefe )
                                        ...
                                        @else
                                            @if(auth()->user()->hasRole('Seguridad e Higiene'))
                                                ...
                                                @if (strtoupper(auth()->user()->name) == $vacacion->Nom_Emp.' '.$vacacion->Ap_Pat.' '.$vacacion->Ap_Mat)
                                                  <option value={{$i->No_Empleado}}>{{$i->Nom_Emp.' '.$i->Ap_Pat.' '.$i->Ap_Mat}}</option>
                                                @else


                                                        <option value={{auth()->user()->no_empleado}}>{{strtoupper(auth()->user()->name)}}</option>
...
<script>
    $(document).ready(function() {
        if($('#eventualidades').val().substr(0,1) >= $('#eventualidades').val().substr(2,1)  && $('#periodos').val().substr(0,1) >= $('#periodos').val().substr(2,1) ){
               ...
               if( validadorFecha3==1 )
                {
                    ...
                    $.ajax({
                        url: 'vacaciones.getajax4',
                        method: 'POST',
                        data:{

                            fecha2: $('#fin_vac').val(),
                            fecha1: $('#inicio_vac').val(),
                            _token:$('input[name="_token"]').val()
                        }
                    }).done(function(res){
                        if(res>0){
